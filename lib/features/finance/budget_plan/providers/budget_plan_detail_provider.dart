// lib/features/finance/budget_plan/providers/budget_plan_detail_provider.dart

import 'package:flutter/foundation.dart';
import '../data/models/budget_plan_model.dart';
import '../data/repositories/budget_plan_repository.dart';

class BudgetPlanDetailProvider extends ChangeNotifier {
  final _repo = BudgetPlanRepository.instance;

  BudgetPlanDetailModel? _detail;

  bool _loading = false;
  String? _error;
  bool _acting = false;

  BudgetPlanDetailModel? get detail => _detail;
  ...
  Future<bool> reject(String id, String reason) async {
    ...
    notifyListeners();
    try {
      await _repo.returnPlan(id, reason);
      await load(id);
      return true;
    } catch (e) {
      _error = e.toString();
